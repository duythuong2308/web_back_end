    ...
    for (let row = 0; row < array.length; row++) {
        ...
        for (let col = 0; col < keys.length; col++) {
            ...
            if (keys[col] === "Id") {
                ...
                a.href = `/gui/commune.html?districtId=${value}`;
                td.appendChild(a);
            } else {
        ...
        x.id = `deleteDistrict${array[row]["Id"]}`;
        ...
        x.addEventListener("click",function () {
            ...
            xhr.open("DELETE", '/api/district', true);
...
window.onload = async () => {
    ...
    let resp = await FetchBackendRows(GetBackend(),
        "/api/district"+window.location.search);
    loadArrayToTable(document.getElementById("districts"), resp["Data"]);
};
